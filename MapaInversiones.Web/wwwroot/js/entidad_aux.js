  ...
  $.ajax({
    ...
    data: {
      ...
      if (data.presupuestoVigenteAnnioDisplay) { 
          ...
          + '                <div class="h1" id="PresupuestoVigente">RD$ ' + ((data.presupuestoVigenteAnnioDisplay *1)/1000000).formatMoney(2, ',', '.').toString().trim() +' millones </div>   '
          + '            </div>                                                                                                                                        '
          + '        </div>                                                                                                                                            '
          + '    </div>                                                                                                                                                '
          + '</div>                                                                                                                                                   ';
      ...
      if (data.presupuestoEjecutadoAnnioDisplay) { 
          ...
          + '                <div class="h1" id="PresupuestoEjecutado">RD$ ' + ((data.presupuestoEjecutadoAnnioDisplay * 1) / 1000000).formatMoney(2, ',', '.').toString().trim() +' millones</div>'
          + '            </div>                                                                                                                                        '
          + '        </div>                                                                                                                                            '
          + '    </div>                                                                                                                                                '
              + '</div>                                                                                                                                                    ';
      ...
      if (data.porcEjecutadoAnnioDisplay) {
          html += '<div class="col-lg-4 mb-3">                                                                                                                               '
              ...
              + '                <div class="h1" id="PorcEjecutado">RD$ ' + ((data.porcEjecutadoAnnioDisplay * 1)).formatMoney(2, ',', '.').toString().trim() +' %</div>                              '
              + '            </div>                                                                                                                                        '
              + '        </div>                                                                                                                                            '
              + '    </div>                                                                                                                                                '
          '</div>                                                                                                                                                     ';
...
function getEstructuraInfograficoNew(datos, pagina) {
    ...
    for (var i = 0; i < datos.length; i++) {
        ...
        html_str += '<span class="td1">RD$ ' + (valor_proyecto*1/1000000).formatMoney(2, ',', '.').toString() + ' Millones</span>';
        html_str += '</div>';
        ...
        for (var j = 0; j < lineas_vec.length; j++) {
            ...
            html_str += '<span class="td1p">' + 'RD$' + ' ' + presup_aprobado.formatMoney(1, ',', '.').toString() + ' Millones</span>';
            html_str += '</div>';

            html_str += '<div class="data1">';
            html_str += '<span class="labelTit">Presupuesto Vigente</span>';
            html_str += '<span class="td1p">' + 'RD$' + ' ' + presup_vigente.formatMoney(0, ',', '.').toString() + ' Millones</span>';
            html_str += '</div>';

            html_str += '<div class="data1">';
            html_str += '<span class="labelTit">Ejecución Acumulada</span>';
            html_str += '<span class="td1p">' + 'RD$' + ' ' + presup_ejecutado.formatMoney(0, ',', '.').toString() + ' Millones</span>';
            html_str += '</div>';
...
function getEstructuraInfograficoPerLineas(datos_lineas, pagina) {
    if (datos_lineas.length > 0) {
        ...
        for (var j = 0; j < datos_lineas.length; j++) {
            ...
            str_lineas += '<span class="td1">RD$ ' + presup_aprobado.formatMoney(2, '.', ', ').toString() + ' Millones</span>';
            str_lineas += '</div>';
            str_lineas += '<div class="data1">';
            str_lineas += '<span class="labelTit">Presupuesto Vigente</span>';
            str_lineas += '<span class="td1">RD$ ' + presup_vigente.formatMoney(2, '.', ', ').toString() + ' Millones</span>';
            str_lineas += '</div>';
            str_lineas += '<div class="data1">';
            str_lineas += '<span class="labelTit">Ejecución Acumulada</span>';
            str_lineas += '<span class="td1">RD$ ' + presup_ejecutado.formatMoney(2, '.', ', ').toString() + ' Millones</span>';
            str_lineas += '</div>';
            str_lineas += '<div class="data1">';
                ...
                tooltipConfig: {
                    ...
                    tbody: [
                        [function (d) {
                            ...
                            var cad = "";
                            cad += "<span>Presupuesto Vigente " + "RD$ " + valor.formatMoney( 0, '.', ',').toString() + " Millones" + "</span></br>";
                            return cad;
                        }]
                    ]
                },
            ...
            tooltipConfig: {
                ...
                tbody: [
                    [function (d) {
                        ...
                        var cad_aux = "RD$ " + d["rawValue"].formatMoney(1, ',', '.').toString() + " ";
                        if (d["porcentaje"] != undefined && d["porcentaje"] != null) {
                            cad_aux = "RD$ " + d["rawValue"].formatMoney(1, ',', '.').toString() + " " + " <strong>(" + d["porcentaje"].formatMoney(1, '.', ',').toString() + " %)</strong>";
                        }
                        return cad_aux;
...
function loadConsolidaGastoEntidad(data, div) {
    ...
    for (var i = 0; i < data.length; i++) {
        txt_aux += "<div class='box' style='background:linear-gradient(to right, " + assignColorPaletaD(i) +" 20px, #f2f2f2 0) !important;'>"
            + "<div class='h3'>RD$ " + (data[i].rawValue*1).formatMoney(1, ',', '.').toString() + "</div>"
...
function GetListadoInstituciones(institucionesPorPagina) {
    ...
    for (var i = 0; i < institucionesPorPagina.length; i++) {
        ...
        html_list += '<div class="data1"><span class="labelTit">Monto estimado</span><span class="td1">RD$ ' + institucionesPorPagina[i]['montoEstimado'].formatMoney(2, '.', ',').toString() + ' </span></div>';
        html_list += '</div>';
